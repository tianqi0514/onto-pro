...
| SimulationService | 后续模块，负责 What-if 参数扰动、风险事件注入、规则重跑、差异对比和影响路径输出 |
...
模拟推演不另建一套金融业务逻辑，必须复用项目事实、本体对象、规则、Skill 和证据链。
...
sequenceDiagram
  ...
  U->>API: 提交推演模板和参数变化
  API->>SIM: 创建 SimulationRun
  SIM->>RP: 读取项目事实快照
  SIM->>SIM: 注入参数扰动或模拟事件
  SIM->>SK: 调用必要 Skill 重新计算指标
  SIM->>RL: 重跑相关规则
  SIM->>SIM: 对比推演前后差异
  SIM->>EV: 组装假设、证据和影响路径
  SIM->>RP: 保存 SimulationRun
  SIM-->>API: 返回模拟结果和人工复核建议
```
...
| 模拟推演中心 | 推演模板、参数变化、结果差异、影响路径 | 批量压力测试、组合策略仿真 |
...
后续模块，一期可先返回 mock 数据和开发中状态。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/simulations/templates` | 推演模板列表 |
| GET | `/api/projects/{project_id}/simulation-templates` | 某项目可用推演模板 |
| POST | `/api/simulations/runs` | 创建 SimulationRun |
| GET | `/api/simulations/runs/{simulation_run_id}` | 推演结果详情 |
| POST | `/api/simulations/runs/{simulation_run_id}/save-as-eval-case` | 将关键推演边界保存为 EvalCase |
...
```json
{
  ...
  "project_id": "project_qsl_001",
  "template_id": "sim_template.collateral_value_down",
  "status": "completed",
  "assumptions": [
    {
      "name": "抵押物评估价值",
      "before_value": "100000000",
      "simulated_value": "80000000",
      "source": "user_input"
    }
  ],
  "diff": [
...
| eval_results | 评测结果 |
| feedback_items | 用户反馈 |
| simulation_scenarios | 推演模板 |
| simulation_runs | 推演运行 |
| simulation_diffs | 推演前后差异 |
...
| Sprint 6 | 模拟推演预研 | 单项目 What-if、参数扰动、规则重跑、结果差异 |
...
| 复杂批量仿真 | 先做单项目 What-if，等结果表和评测闭环稳定后再做多项目压力测试 |
