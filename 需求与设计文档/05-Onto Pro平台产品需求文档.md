...
| 编号 | 优先级 | 需求 |
| --- | --- | --- |
| FR-801 | P0 | AgentRun 必须记录用户、时间、问题、模型、本体版本、Prompt 版本、Skill 调用、证据和结论 |
| FR-802 | P0 | 本体和规则变更必须记录创建人、审核人、发布时间和变更摘要 |
| FR-803 | P0 | 支持敏感字段标记，例如身份证、手机号、征信报告、公章、个人信息 |
| FR-804 | P0 | 高风险动作默认禁用自动执行 |
| FR-805 | P1 | 支持角色权限：管理员、业务专家、项目用户、只读用户 |
| FR-806 | P1 | 支持脱敏视图和原文视图权限控制 |
| FR-807 | P2 | 支持企业审计导出和合规留痕 API |
...
1. 第一个开发迭代是否以 Markdown/本地文件样板为主，还是直接接入数据库和对象存储？
2. 金融客户材料是否允许进入开发环境，是否需要先做脱敏样本包？
3. OCR/视觉能力是自研、接第三方，还是先用人工标注结果模拟？
4. 客户统一业务系统是否提供项目台账、材料清单、流程状态和报表接口？
5. 第一批外部查询 Skill 是否接工商、司法、中登、发票，还是先用离线样例？
6. 本体审核角色由客户业务专家承担，还是由内部交付团队先代管？
7. 评测通过率达到什么门槛后允许客户试用？
